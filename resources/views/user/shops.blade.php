								...
								<div class="grid-inner bg-white row g-0 p-2 border-0 rounded-5 shadow-sm h-shadow all-ts h-translate-y-sm">
									<div class="col-12 mb-md-0">
										<div class="entry-image mb-2">
											<img src="{{ url('assets/oleh-oleh/'. $shop->picture) }}" alt="Inventore voluptates velit totam ipsa tenetur" class="rounded-5">
									...
									<div class="col-12 p-4 pt-0 pb-1">
										...
										<div class="entry-meta no-separator mb-3">
											<ul>
												<li class="fw-normal"><i class="uil text-warning uil-map-marker"></i> {{ $shop->address }}</li>
											</ul>
										</div>
										{{-- fasii;litas --}}
										{{-- <div class="mb-3">
										...
										<div class="mb-3">
											<div class="fw-bold">Rp. <?= number_format($shop->price, 0, ",", ".") ?></div>
										</div>
										<div class="entry-meta no-separator mb-3">
											<ul>
												<li class="fw-normal text-warning"><i class="uil bi-telephone-fill"></i> {{ $shop->phone }}</li>
											</ul>
										</div>
										{{-- gk ada detail --}}
										...
										@if($shop->link_youtube != null || $shop->link_instagram != null || $shop->link_facebook != null || $shop->link_tiktok != null)
											<div class="entry-meta no-separator mb-3">
												<ul>
													@if($shop->link_youtube != null)
														<li><a href="{{ $shop->link_youtube }}" class="fw-normal text-dark"><i class="uil uil-youtube"></i> </a></li>
													@endif
													@if($shop->link_instagram != null)
														<li><a href="{{ $shop->link_instagram }}" class="fw-normal text-dark"><i class="uil bi-instagram"></i> </a></li>
													@endif
													@if($shop->link_facebook != null)
														<li><a href="{{ $shop->link_facebook }}" class="fw-normal text-dark"><i class="uil uil-facebook"></i> </a></li>
													@endif
													@if($shop->link_tiktok != null)
														<li><a href="{{ $shop->link_tiktok }}" class="fw-normal text-dark"><i class="uil fa-brands fa-tiktok"></i> </a></li>
													@endif
												</ul>
											</div>
										@else
